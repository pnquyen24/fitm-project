import { Box, Button, Chip, Grid, MenuItem, OutlinedInput, Select, TextField, useTheme } from "@mui/material";
...
function getStyles(name, personName, theme) {
    return {
      fontWeight:
        personName.some(pName => pName.id === name.id)
  ...
  function removeDuplicates(array) {
    ...
    let uniqueIds = ids.filter((id, index) => ids.indexOf(id) === ids.lastIndexOf(id));
...
function RoleModify() {
    ...
    function GetMemberRoles() {
        axiosClient.get(`${GET}?id=${id}`)
        .then(res => res.data)
        .then(data => {
            setMemberInfo({id: data.id, fullname: data.name, username: data.username});
    ...
    const handleChange = (event) => {
      ...
      const handleSave = (event) => {
        axiosClient.post(ADD, {userId: memberInfo.id, roleIds: personRole.map(role => role.id)})
    ...
    return (
        <Grid paddingTop={10} container xs={6} spacing={2}>
            ...
            <Grid item xs={4}>
                <TextField label="Full Name" aria-readonly value={memberInfo.fullname}/>
            </Grid>
            <Grid item xs={12}>
                <Select
                    ...
                    renderValue={(selected) => (
                        selected.length === 0 ? <em>Roles</em>
                        :<Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, backgroundColor:"white"}}>
                            {selected.map((value) => (
            ...
            <Grid item xs={12}>
                <Button variant="outlined" onClick={e => handleCancel(e)}>Cancel</Button>
